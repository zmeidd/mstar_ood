    ...
    def Train(self, input_spikes, label, bs, logits):
        ...
        for t in range(1, self.T):
            ...
            if not self.online_norm:
                ...
                for h in range(1, n_hidden):
                    psp =oe.contract("Bi,ij->Bj", hidden_spikes[h - 1][t - 1, :, :], self.w_h[h])

                    eps = 5
                    N,D = psp.shape
                    sample_mean = psp.mean(axis=0)
                    sample_var = psp.var(axis=0)
                    std = np.sqrt(sample_var + eps)
                    x_centered = psp - sample_mean
                    x_norm = x_centered / std

                    psp_out = psp -x_norm

                    U_h[h][t, :, :] =  psp_out+ U_h[h][t - 1, :, :] + self.threshold_h[h] * self.bias
                    hidden_spikes[h][t, :, :] = U_h[h][t, :, :] >= self.threshold_h[h]
                    U_h[h][t, hidden_spikes[h][t, :, :]] = 0
                    U_h[h][t, :, droph[h]] = 0
            ...
            if (t > (self.twin)):
                ...
                if t>(self.twin * 2):
                    ...
                    if self.evt == 1:
                        ...
                        tmp0 = (np.sum(hidden_spikes[n_hidden - 1][int(t - self.twin + 1 - self.twin): int(t + 1 - self.twin), :, :],
                ...
                if self.dr:
                    if self.dfa != 2:
                        derivative = (np.sum(hidden_spikes[0][0:int(self.twin), :, :], axis=0, keepdims=False) > 0)
