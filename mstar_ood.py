...
if 'cifar10_' in args.method_name:
    test_data = dset.CIFAR10('./data', download = True, train=False, transform=test_transform)
    num_classes = 10
else:
    ...
    test_data = dset.ImageFolder('/home/bwang30/study/ann/mstar_with_machine_learning/MSTAR-10/newtask2test', temp_transform)
...
if args.load != '':
    for i in range(1000 - 1, -1, -1):
        ...
        model_name = "/home/bwang30/study/snn/outlier-exposure/CIFAR/snapshots/mstar/mstar_wrnmstar32_newtask2_epoch_199.pt"
...
def get_ood_scores(loader, in_dist=False):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        outputs = outputs   # temperature scaling

#//////////////////input preprocessing //////////////////////////
        inputs = Variable(data.cuda(CUDA_DEVICE),requires_grad = True)
        inputs.retain_grad()
        optimizer = optim.SGD(net.parameters(), lr = 0, momentum = 0)
        outputs = net(inputs[:, :1, :, :])

        targetcu = Variable(target.cuda(CUDA_DEVICE)).long()
        # print(outputs.min())
        # outputs = outputs / 1000
        loss = F.cross_entropy(outputs, targetcu)
        net.zero_grad()
        loss.backward()
        # optimizer.step()
        ...
        labels = torch.autograd.Variable(torch.LongTensor([maxIndexTemp]).cuda(CUDA_DEVICE))
        gradient =  torch.ge(inputs.grad, 0)
        ...
        tempInputs = torch.add(inputs.data,  -0.0014, gradient)
        outputs = net(Variable(tempInputs[:, :1, :, :]))
        # outputs = outputs / 1000
        # smax = to_np(F.softmax(outputs, dim=1))
...
#//////////////////input preprocessing end //////////////////////////
        ...
        smax = to_np(F.softmax(outputs, dim=1))
        if args.use_xent:
            _score.append(to_np((outputs.mean(1) - torch.logsumexp(outputs, dim=1)))) # input preprocessing
            # print(to_np((outputs.mean(1) - torch.logsumexp(outputs, dim=1))))
            # exit()
            # _score.append(to_np((output.mean(1) - torch.logsumexp(output, dim=1))))

        else:
            _score.append(-np.max(smax, axis=1))
            # print(-np.max(smax, axis=1))
            # exit()
        if in_dist:
            ...
            if args.use_xent:
                _right_score.append(to_np((outputs.mean(1) - torch.logsumexp(outputs, dim=1)))[right_indices])
                _wrong_score.append(to_np((outputs.mean(1) - torch.logsumexp(outputs, dim=1)))[wrong_indices])
...
# ///////////////  mstar split ///////////////
temp_transform = trn.Compose([trn.Resize([32,32]),trn.ToTensor()])    # 0~255 --> 0~1
ood_data = dset.ImageFolder('/home/bwang30/study/ann/mstar_with_machine_learning/MSTAR-10/newtask1test', temp_transform)
...
#                      transform=trn.Compose([trn.Resize([32,32]), trn.ToTensor()]), download=False)
# ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
#                                          num_workers=args.prefetch, pin_memory=True)
